...
const initialProducts = [
  { id: 1, nombre: "Producto 1", precio: 100, cantidad: 50, estado: "Disponible" },
  { id: 2, nombre: "Producto 2", precio: 150, cantidad: 30, estado: "Agotado" },
];

// Constantes y utilidades
const COMPANY_NAME = "Extractus";
const REPORT_TITLE = "Reporte de Productos";
...
const allColumns = ["ID", "Nombre", "Precio", "Cantidad", "Estado"];
...
// Componente principal para mantenimiento de productos
const MantenimientoProductos = () => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const toast = useToast();
  const [productos, setProductos] = useState(initialProducts);
  const [selectedProducto, setSelectedProducto] = useState(null);
  const [filters, setFilters] = useState({ nombre: "", precio: "", estado: "" });
  ...
  const handleSaveProducto = () => {
    if (selectedProducto.id) {
      ...
      setProductos((prevProductos) => [
        ...prevProductos,
        { ...selectedProducto, id: productos.length + 1 },
      ]);
    }
    onClose();
  ...
  return (
    <Box p={5}>
      <Heading mb={6}>Mantenimiento de Productos</Heading>
      ...
      <HStack spacing={3} justify="flex-end" mb={4}>
        <Button
          colorScheme="green"
          onClick={() => {
            setSelectedProducto({ id: null, nombre: "", precio: "", cantidad: 0, estado: "Disponible" });
            onOpen();
          }}
          leftIcon={<FaPlus />}
        >
        ...
        <Menu>
          ...
          <MenuList>
            <MenuItem
              icon={<FaFilePdf />}
              onClick={() => exportToPDF(filteredProductos, allColumns)}
            >
              Exportar a PDF
            </MenuItem>
            <MenuItem
              icon={<FaFileExcel />}
              onClick={() => exportToExcel(filteredProductos, allColumns)}
            >
              Exportar a Excel
            </MenuItem>
          </MenuList>
