// ============================================================
// 📁 src/components/Mantenimiento/MantenimientoEstadoProveedor.js
// ============================================================
...
export default function MantenimientoEstadoProveedor() {
  // ============================================================
  ...
  // ============================================================
  const cargarEstadosProveedor = useCallback(async () => {
    try {
      const res = await api.get("/mantenimiento/estado-proveedor");
      setData(res.data);
    } catch (err) {
      console.error("❌ Error cargando estados de proveedor:", err);
      toast({
  ...
  const fields = [
    {
      name: "nombre_estado",
      label: "Nombre del Estado del Proveedor",
      type: "text",
      required: true,
    },
  ];
    ...
    <>
      ...
      <Box overflowX="auto">
        <CrudTabla
          ...
          extractors={{
            ID: (r) => r.id_estado_proveedor,
            "Nombre del Estado": (r) => r.nombre_estado,
          }}
          fields={fields}
          idKey="id_estado_proveedor"
          initialData={data}
          onReload={cargarEstadosProveedor}
          apiUrl="/mantenimiento/estado-proveedor"
        />
      </Box>
    </>
  );
